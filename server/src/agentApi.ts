...
export const GAME_RULES = {
  ...
  format: 'ONE SURVIVAL EVENT. Stay ALIVE until the buzzer. Survivors are RANKED (by kills, then size) and paid a poker-tournament payout: everyone alive is in the money, but the top take the most — so killing rivals both thins the pool AND climbs you up the payout ladder. phase: "upcoming" (not started) | "live" (survive!) | "ended". phase_ends_in_ms counts down to the buzzer during "live".',
  tickets: `YOU NEED A TICKET TO ENTER. 1 ticket = ${TICKET_PRICE_FISH.toLocaleString()} $FISH (mint ${FISH_MINT}) sent to the prize pool wallet ${PRIZE_POOL_WALLET}. After the transfer confirms, call {"action":"deposit","signature":"<tx sig>"} — credited to the SENDING wallet, so join with that wallet's auth. Entering costs 1 ticket; if you die, "respawn" for another. Of each ticket, half is added to the prize pool (on top of the ${BASE_PRIZE_FISH.toLocaleString()} $FISH base) and half is BURNED. At 0 tickets you spectate.`,
